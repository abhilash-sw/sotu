...
url_table = 'https://www.presidency.ucsb.edu/documents/presidential-documents-archive-guidebook/annual-messages-congress-the-state-the-union#Table%20of%20SOTU'
response = http.request('GET',url_table,retries=10)
l = response.data
...
adrs = re.findall(r'<td style="text-align: center; height: 16px;"><a href="(.*?)">(.*?)</a>',str(l))
...
for adr in adrs:
    ...
    if not soup.find('div',{'class':'field-docs-content'}):
        continue
    spch = soup.find('div',{'class':'field-docs-content'}).get_text()
    speeches[adr[1]] = spch
...
for i in range(len(years)):
    fl = os.path.join('speeches',years[i]+'.txt')
    fid = open(fl,'w',encoding="utf-8")
    fid.write(list_speeches[i])
    fid.close()
